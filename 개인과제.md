...
![image](https://user-images.githubusercontent.com/74235341/105262406-7b500180-5bd3-11eb-9d0b-fbd444fb9449.png)
...
```
![image](https://user-images.githubusercontent.com/74235341/105258596-6ec8aa80-5bcd-11eb-8f97-fa2c1411ead0.png)
```
# Order 서비스의 취소처리
http PATCH http://order:8080/orders/1 status="Cancel"
```
![image](https://user-images.githubusercontent.com/74235341/105258604-79833f80-5bcd-11eb-9e14-38fe55e468ba.png)
```
# 주문 상태 확인
http http://52.231.10.38:8080/mypages
http http://customercenter:8080/mypages
```
![image](https://user-images.githubusercontent.com/74235341/105258639-89028880-5bcd-11eb-8649-941510f8b624.png)
...
```
# 취소 (delivery) 서비스를 잠시 내려놓음 

kubectl scale deploy cancel --replicas=0
...
![image](https://user-images.githubusercontent.com/74235341/105259236-c4518700-5bce-11eb-955a-a7e56af77898.png)


![image](https://user-images.githubusercontent.com/74235341/105259283-dfbc9200-5bce-11eb-9818-0fd50a3c6fa5.png)
...
- 피호출 서비스(취소:cancellation) 의 임의 부하 처리 - 500 밀리에서 증감 220 밀리 정도 왔다갔다 하게, Thread.currentThread().sleep((long) (500 + Math.random() * 220));
```
...
```
siege -c150 -t20S -v --content-type "application/json" 'http://order:8080/orders/1 PATCH {"status": "Cancel"}'

```

- 새버전으로의 배포 시작
```
kubectl set image deployment/cancel cancel=cloth.azurecr.io/delivery:latest --record
```

- seige 의 화면으로 넘어가서 Availability 가 100% 미만으로 떨어졌는지 확인

![image](https://user-images.githubusercontent.com/74235341/105180341-cb947880-5b6d-11eb-9f9e-83c62a57eab7.png)



배포기간중 Availability 가 평소 100%에서 92.11% 대로 떨어지는 것을 확인. 원인은 쿠버네티스가 성급하게 새로 올려진 서비스를 READY 상태로 인식하여 서비스 유입을 진행한 것이기 때문. 이를 막기위해 Readiness Probe 를 설정함:
